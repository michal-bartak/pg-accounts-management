...
- **Create role grants privileges per cluster/group**, using the same scope editor as
  Alter role (no plain "Parent role" field). Pending grants live in
  `createParents: Map<role, Set<clusterId>>`; the scope dialog runs in a **create mode**
  (`ctx.mode === 'create'`) whose cluster universe is `createUniverse()` (the sidebar
  target selection). Run emits **one `create_role` per selected cluster** with only that
  cluster's scoped parents (`buildCreateRequests` → `executeAlterRequests`); a cluster
  with no scoped parents is still created with the template's base groups.

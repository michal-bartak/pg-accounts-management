...
5. Tag: `git tag -a v0.1.0 -m "pgCowboy 0.1.0"`.
...
Published from [pgcowboy](https://github.com/michal-bartak/pgcowboy) via [`.github/workflows/release.yml`](.github/workflows/release.yml). Builds **Linux**, **Windows**, and **macOS** artifacts on native GitHub runners when you run the workflow manually.
...
2. Create and push the tag:
   ```bash
   git tag -a v0.2.0 -m "pgCowboy 0.2.0"
...
|----------|------|----------|
| macOS universal | `pgCowboy-v{VERSION}-macos-universal.dmg` | [`build/scripts/make-dmg.sh`](build/scripts/make-dmg.sh) (`hdiutil`) |
| Windows amd64 | `pgCowboy-v{VERSION}-windows-amd64.msi` | [`build/scripts/make-msi.ps1`](build/scripts/make-msi.ps1) (WiX v3, [`product.wxs`](build/windows/installer/product.wxs)) |
| Linux amd64 (deb) | `pgCowboy-v{VERSION}-linux-amd64.deb` | [`build/scripts/make-linux-packages.sh`](build/scripts/make-linux-packages.sh) (fpm) |
| Linux amd64 (rpm) | `pgCowboy-v{VERSION}-linux-amd64.rpm` | same script |
...
1. Open the `.dmg` and drag **pgCowboy** to Applications.
2. Clear the quarantine flag once: `xattr -dr com.apple.quarantine /Applications/pgCowboy.app`
...
| OS | Example |
|----|---------|
| macOS | `dist/pgCowboy-v0.1.0-macos-arm64.dmg` (`macos-universal` with `PLATFORM=darwin/universal`) |
| Linux | `dist/pgCowboy-v0.1.0-linux-amd64.deb`, `dist/pgCowboy-v0.1.0-linux-amd64.rpm` |
| Windows | `dist/pgCowboy-v0.1.0-windows-amd64.msi` |
